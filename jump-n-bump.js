...
var CustomLevel = function CustomLevel(name, filename, load) {
    this.name = name;
    this.filename = filename;
    this.load = load || function () {
        return fetch('levels/' + this.filename + '/' + this.filename + '.dat').then(function (r) {
            return r.blob();
        }).then(function (file) {
...
function ViewModel() {
    ...
    this.availableLevels = _knockout2.default.observableArray([new CustomLevel("Original", "default", function () {
        return Promise.resolve((0, _default_levelmap.create_default_level)());
    }), new CustomLevel("Caves", "caves"), new CustomLevel("Cocaine", "cocaine"), new CustomLevel("Green", "green"), new CustomLevel("Jump2", "jump2"), new CustomLevel("King of the Hill", "kingofthehill"), new CustomLevel("Mario", "mario"), new CustomLevel("S General", "sgeneral"), new CustomLevel("Spring", "spring"), new CustomLevel("Swamp", "swamp"), new CustomLevel("Thomas", "thomas"), new CustomLevel("Topsy", "topsy"), new CustomLevel("Waterfall", "waterfall")]);
    ...
    this.selected_custom_level = _knockout2.default.observable(this.availableLevels()[0]);

    this.current_page = _knockout2.default.pureComputed(function () {
        return self.current_game().game_state();
    });

    this.scores_viewmodel = _knockout2.default.pureComputed(function () {
    ...
    this.load_level_from_file = function (filepath, filedata) {
        ...
        var customLevel = new CustomLevel(filename, filename, function () {
            return _this.get_data_from_filename(filedata).then(function (array) {
                return loader.load(array);
            });
        });
    ...
    this.load_customlevel = function (customLevel) {
        ...
        customLevel.load().then(function (level) {
